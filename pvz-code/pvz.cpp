...
void PvZ::GenGoldenCup() {
    code.asm_init_newThread();
    int a = ReadMemory<int>(base,0x780,0x154);
    code.asm_mov_dword_ptr_esp_add(0x8, 0x0);
    code.asm_mov_dword_ptr_esp_add(0x4, 0x2);
    code.asm_mov_dword_ptr_esp_add(0x0, a);
    code.asm_call(0xa6164);
    //    int p = pvz.ReadMemory<int>(pvz.base,0x780);
    //    pvz.code.asm_mov_dword_ptr_esp_add(0x4, -1);
    //    pvz.code.asm_mov_dword_ptr_esp_add(0x0, p);
    //    pvz.code.asm_call(0x11714);
    code.asm_ret();
    code.asm_create_thread();
}
void PvZ::PatchRedNutExplode() {
    code.asm_init_codeInject();
